...
//	}()
//
//	u, _ := url.Parse(fmt.Sprintf("%v/", ts.URL))
//	p := &dummyProducer{msgs: make(map[string]producer.Message)}
//	c := &dummyHttpClient{}
//	s := newPublishService(u, p, c)
//	theJob, _ := s.newJob("organisations", nil, u, "", 1)
//	for i := 0; ; i++ {
//		if i > 10 {
//			t.Error("Job did not complete")
//			return
//		}
//		status, _ := s.getJobStatus(theJob.JobID)
//		if status == completed {
//			break
//		}
...
//	}
//
//	assert.EqualValues(t, job{URL: *u, ConceptType: "organisations", Count: 2, Throttle: 1, Progress: 2, Status: completed}, theJob)
...
//func TestPublishServiceNoIdsLookup(t *testing.T) {
//
//	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
//		if strings.Contains(r.URL.Path, uuid1) {
//			fmt.Fprintf(w, "{\"uuid\":\"%s\",\"properName\":\"Org1\"}\n", uuid1)
...
//	jobID := s.newJob("organisations", []string{uuid1, uuid2}, u, "", 1)
//	var job jobStatus
//	for i := 0; ; i++ {
//		if i > 10 {
//			t.Error("Job did not complete")
...
//	}
//
//	assert.EqualValues(t, jobStatus{URL: ts.URL + "/", IDS: []string{uuid1, uuid2}, Concept: "organisations", Count: 2, Throttle: 1, Done: 2, Status: "Completed"}, job)
//	assert.Equal(t, 2, len(p.msgs), "Should have produced 2 messages")
//	assert.Equal(t, uuid1, p.msgs[uuid1].Headers["Message-Id"])
//	assert.Equal(t, "organisations", p.msgs[uuid1].Headers["Message-Type"])
//	assert.Equal(t, "http://cmdb.ft.com/systems/upp", p.msgs[uuid1].Headers["Origin-System-Id"])
